...
public class MvpnWebViewOwnerImpl extends Handler implements MvpnWebViewOwner {
    ...
    private static volatile boolean startTunnelAborted = false;
    private static volatile boolean needEnableWebViewClient = false;
    private final Activity mActivity;
    private final List<WebViewRequest> requests;
    private Logger logger = Logger.getLogger(TAG);
    private WebViewRequest mSavedWebViewRequest;
    ...
    public void checkAndStartTunnel() {
        if(MicroVPNSDK.isNetworkTunnelRunning(mActivity)) {
            return;
        }
        if (startTunnelAborted) {
            logger.debug1(TAG, "checkAndStartTunnel");
            runStartTunnel();
        }
    }
    ...
    public void handleMessage(Message msg) {
        ...
        switch (code) {
            case START_TUNNEL_SUCCESS:
                ...
                if (requests.size() == 0 && mSavedWebViewRequest != null) {
                    requests.add(mSavedWebViewRequest);
                }
                needEnableWebViewClient = true;
                MvpnWebViewOwnerImpl.instance.catchUp();
                break;
            case START_TUNNEL_FAILED:
                startTunnelAborted = true;
                MvpnWebViewOwnerImpl.instance.catchUp();
    ...
    public boolean registerWebView(OwnedMvpnWebView toRegister) {
        ...
        if (isAlreadyRegistered && !needEnableWebViewClient) {
            return true;
        }
        ...
        try {
            ...
            if(MicroVPNSDK.isNetworkTunnelRunning(mActivity)) {
                if(toRegister.getWebViewClient() != null && !(toRegister.getWebViewClient().getClass().getName().contains("InAppBrowser$"))) {
                    MicroVPNSDK.enableWebViewObjectForNetworkTunnelForCordova(mActivity, toRegister.getWebView(), toRegister.getWebViewClient());
                } else {
