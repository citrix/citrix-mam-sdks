...
function readBuildJson(defaultProperties) {
	...
	// get user defined path for managed_app_utility.jar
	let wrapper_loc = ''
    if (build.hasOwnProperty('mdxJarPath')) {
        wrapper_loc = build['mdxJarPath'];
    } else {
        wrapper_loc = 'platforms/android/app/src/main/mdx/managed-app-utility.jar'
    }
	// for convenience
	build = build['android'];
